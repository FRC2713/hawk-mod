...
**`TOKEN_ENCRYPTION_KEY` is not recoverable.** Lose it and every mentor has to
...
`{"status":"ok","installed":false,"enrolledMentors":0}` is the expected state
...
4. Send every mentor the same install URL, and watch `/hawkmod status` until
   coverage reads N/N.
...
- Rotate `TOKEN_ENCRYPTION_KEY` by having mentors re-enroll if the host is ever
