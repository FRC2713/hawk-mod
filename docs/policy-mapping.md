...
| 4.1 | No 1:1 mentor–student DMs, ever                                               | `dmPolicy` `one_to_one_adult_student`, raised on first message and on backfill                                                                          |
...
| 5   | Business+ / Corporate Export                                                  | procurement — **manual**. Note hawk-mod does not depend on Corporate Export; it is the backstop for mentors who never enroll                            |
...
| 6   | User group editing restricted to Owners/Admins                                | workspace setting — **manual**. Only matters when `STUDENT_USERGROUP`/`MENTOR_USERGROUP` are set, since group membership then declares who is monitored |
...
- **No archive of adult-only conversations.** Mentor-to-mentor DMs are outside
